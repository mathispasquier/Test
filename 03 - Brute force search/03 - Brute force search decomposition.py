...
# Need to redefine POA irradiance with the irradiance from the actual data (but optimized angles from the decomposition model)
surface_azimuth_function = lambda x: 90 if x<0 else 270
surface_azimuth = diffuse_tracking_erbs['angle'].apply(surface_azimuth_function, axis=1)
"""diffuse_tracking_erbs['POA global'] = pvlib.irradiance.get_total_irradiance(surface_tilt=diffuse_tracking_erbs['angle'].apply(np.abs), 
                                                     surface_azimuth=surface_azimuth, 
                                                     dni=DNI, 
                                                     ghi=GHI, 
                                                     dhi=DHI, 
